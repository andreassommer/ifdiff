classdef TestSensitivities < matlab.unittest.TestCase
    ...
    properties (SetAccess=immutable)
        canonexTSpan   = [0 20];
        % try changing these values a little bit and see if the tests still pass.
        canonexX0      = [1; 0];
        canonexP       = 5.437;
        canonexSolver  = 'ode45';
    ...
    methods(Test)
        ...
        function testCanonexEndFull(testCase)
            ...
            % Since this method does not support evaluating sensitivities at SWPs, we use ts+eps(ts)
            % instead.
            % The matrices Gy and Uy computed here are derived analytically from the canonical example.
            tSpan = testCase.canonexTSpan;
            x0    = testCase.canonexX0;
            ...
            Gp_2 = @(t) [0; 0];
            Gp_2_full = @(t) Gy_2(t) * (Uy_1 * Gp_1(t1) + Up_1) + Gp_2(t1);
            testCase.verifyEqual(Gy{3}, Gy_2_full(t1Plus), 'AbsTol', atol2);
            testCase.verifyEqual(Gp{3}, Gp_2_full(t1Plus), 'AbsTol', atol2);
            testCase.verifyEqual(Gy{4}, Gy_2_full(t2Minus), 'AbsTol', atol2);
            testCase.verifyEqual(Gp{4}, Gp_2_full(t2Minus), 'AbsTol', atol2);
            % update matrices at t2
            x2t2 = deval(sol,t2,2);
            ...
            Gp_3 = @(t) [0; 0];
            Gp_3_full = @(t) Gy_3(t) * (Uy_2 * Gp_2_full(t2) + Up_2) + Gp_3(t2);
            testCase.verifyEqual(Gy{5}, Gy_3_full(t2Plus), 'AbsTol', atol3);
            testCase.verifyEqual(Gp{5}, Gp_3_full(t2Plus), 'AbsTol', atol3);
            testCase.verifyEqual(Gy{6}, Gy_3_full(tSpan(2)), 'AbsTol', atol3);
            testCase.verifyEqual(Gp{6}, Gp_3_full(tSpan(2)), 'AbsTol', atol3);
        end
    end

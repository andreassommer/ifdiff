classdef HelpCommand < UserCommand
    methods
            ...
            if (length(argCell) > 1)
                if ~StringUtil.isSimpleString(argCell{2})
                    throw(MException(UserCommand.ERROR_BAD_ARGUMENT, sprintf(...
                        'expected the name of a command, but got %s', StringUtil.toStr(argCell{2}))));
                end
            ...
            if (isfield(commandConfig, 'About'))
                ...
                if isempty(command)
                    throw(MException(UserCommand.ERROR_EXPECTED_EXCEPTION, sprintf( ...
                        'unknown command: %s', commandConfig.About)));
                end
        ...
        function message = generalHelp(this)
            ...
            messageLines = [messageLines, ' ', sprintf( ...
                'run speedtracker(''%s'', <command>) for more info about individual commands', this.getName())];

function output = assembleSensitivityOutput(sensData, t_sort_unique, Gy_flag, Gp_flag, Gmatrices_intermediate)
    % output = assembleSensitivityOutput(sensData, t_sort_unique, Gy_flag, Gp_flag, Gmatrices_intermediate)
    %
    ...
    %        Gmatrices_intermediate      - struct that contains the intermediates and updates w.r.t. y0 and p, or empty
    %
    ...
    for i = 1 : size(sensData, 2)
        ...
        for k = 1 : length(sensData(i).timepoints)
            ...
            if Gy_flag
                ...
                if ~isempty(Gmatrices_intermediate)

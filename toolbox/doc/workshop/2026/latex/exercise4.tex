...
\begin{enumerate}[label=\alph*)]
    ...
    \textbf{Steps:}
    \begin{enumerate}[label=\arabic*.]
       \item Implement the anonymous function \lstinline|solutionFunc = @(p) ...| in the file \texttt{runTask4.m}, which should take a parameter vector and use IFDIFF to compute a solution for the given parameter.
       \item You can use the provided function \lstinline|solutionGrid = solveOnGrid(solutionFunc)| to generate a solution for each parameter in the discretization grid.
       \item You can use the provided function \lstinline|plotPopulationGrid(solutionGrid)| to plot your solutions on a grid.
    \end{enumerate}
    \textbf{Hints:}
    \begin{itemize}
...
func(2, 3) == 10 % true
    ...
    \begin{enumerate}[label=\arabic*.]
       ...
       \item You can use the provided code \lstinline|objectiveGrid = arrayfun(objectiveFunc, solutionGrid)| to apply your objective function to each solution in the grid.
       \item You can use the provided function \lstinline|plotObjective(solutionGrid, objectiveGrid)| to plot your objective function as a 3D surface plot.
    \end{enumerate}
    \textbf{Hints:}
    \begin{itemize}
    ...
    \begin{enumerate}[label=\arabic*.]
       ...
       \item You can use the provided code \lstinline|gradientFunc = @(solution) computeGradient(datahandle, solution)| to define an anonymous function, which takes a solution and returns the gradient.
       \item You can use the provided code \lstinline|gradientGrid = arrayfun(gradientFunc, solutionGrid, 'UniformOutput', false)| to apply your gradient function to each solution in the grid.
       \item You can use the provided function \lstinline|plotGradient(solutionGrid, objectiveGrid, gradientGrid)| to plot your gradients as arrows in a 3D surface plot of the objective function.
    \end{enumerate}
    \textbf{Hints:}
    \begin{itemize}
    ...
    \begin{enumerate}[label=\arabic*.]
        ...
        \item You can use the provided code \lstinline|optimizationFunc = @(p) objWithGrad(p, solutionFunc, objectiveFunc, gradientFunc)| to define an anonymous function which takes a parameter vector and returns the negated objective and gradient.
        \item You can use the provided function \lstinline|[pOpt, objOpt] = optimizeParameters(optimizationFunc)| to setup the optimization problem and compute the optimal parameters and objective value using \lstinline|fmincon|.
        ...
        \item You can use the provided function \lstinline|plotPopulation(solutionOpt)| to plot the solution for the optimal parameters.
    \end{enumerate}
    \textbf{Hints:}
    \begin{itemize}

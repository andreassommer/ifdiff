...
\begin{enumerate}[label=\alph*)]
    ...
    \end{equation}
    The constants $r$ and $C$ specify the growth rate of the population and maximum capacity of the pond. \\\\
    \textbf{Subtask:} \\
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{Plots/task4a}
        \caption{Population plot for subtask \ref{item:task4a}}
    \end{figure}

    \item\label{item:task4b} The population of fish will now be harvested automatically according to the following rule:\\
    ...
    \begin{equation}\label{eq:fish_jump}
        \begin{bmatrix}P(t^+) \\ H(t^+)\end{bmatrix} = \begin{bmatrix}P(t^-) \\ H(t^-)\end{bmatrix} +  \underbrace{\begin{bmatrix}-\alpha P(t^-) \\ \phantom{-}\alpha P(t^-)\end{bmatrix}}_{\Delta}\quad\textbf{whenever} \quad\text{$P(t)$ crosses $T$ from below}
    \end{equation}
    where $P(t^-)$ and $H(t^-)$ denote the state of the solution when the jump condition is triggered and $P(t^+)$ and $H(t^+)$ denote the state of the solution after applying the jump update. \\ \\
    \textbf{Subtask:} \\
    Use IFDIFF to solve the new system for the same constants and initial conditions as in subtask \ref{item:task4a}, including an additional initial condition $H(0) = 0$ for the newly added second solution component. \\
    For the harvesting parameters use $T = 0.5$ and $\alpha = 0.5$. Make sure to pass $p = (T, \alpha)$ as actual parameters to your RHS (do not set them directly in the RHS). \\
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{Plots/task4b}
...
func(2, 3) == 10 % true
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{Plots/task4c}
        \caption{Population grid plot for subtask \ref{item:task4c}}
    \end{figure}

    \item\label{item:task4d} The total amount of fish produced over the time interval $I = [0, 5]$ is given by the objective function
    ...
    \textbf{Hints:}
    \begin{itemize}
       \item A solution struct (as returned by MATLAB integrators and IFDIFF) stores the timepoints at which the solution was computed in ascending order as a vector in the field \lstinline|solution.x|.
       The field \lstinline|solution.y(i, j)| then stores the i-th component of the solution state vector at the timepoint \lstinline|solution.x(j)|.
       You can access the state vector of the solution at the final timepoint in the integration time interval using \lstinline|solution.y(:, end)|.
    \end{itemize}
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{Plots/task4d}
        \caption{Objective function plot for subtask \ref{item:task4d}}
    \end{figure}

    \item\label{item:task4e} The gradient of the objective function $F$ is given by
    ...
    How should we tweak those parameters to obtain an even larger objective function value according to the gradient?
    Remember that the gradient always points in the direction of steepest ascent.\\ \\
    \textbf{Steps:}
    \begin{enumerate}[label=\arabic*.]
       \item Implement the function \lstinline|g = computeGradient(datahandle, solution)| in the file \texttt{runTask4.m}, which should take a datahandle and solution and return the gradient of the objective function for the given solution as described in \cref{eq:fish_gradient}.
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{Plots/task4e}
        \caption{Gradient plot for subtask \ref{item:task4e}}
    \end{figure}

    \item\label{item:task4f} After computing the gradients, we can now optimize the parameters $T$ and $\alpha$ in order to maximize the total amount of fish produced as described by the objective function $F$. However, we will additionaly impose a lower bound constraint on the harvest ratio, namely $\alpha \geq 0.2$, because the number of state jumps becomes arbitrarily large as $\alpha$ goes towards zero, which is intractable to handle numerically.
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{Plots/task4f}
        \caption{Optimized population plot for subtask \ref{item:task4f}}
    \end{figure}
\end{enumerate}

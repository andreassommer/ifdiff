...
datahandle = prepareDatahandleForIntegration(rhs, ...
    'integrator', integrator, 'options', optsIntegrator);
...
plotPopulation(solution);
    ...
    \item\label{item:task4b} If the parameters are chosen such that an actual harvest occurs, then the maximum population is equal to the harvest threshold $T$. Otherwise, the maximum size of the population approaches $C = 1$ asymptotically as before.\\\\
    \textbf{File:} \texttt{rhsTask4.m}
    \begin{lstlisting}
function dy = rhsTask4(~, y, p)
r = 2;
...
plotGradient(solutionGrid, objectiveGrid, gradientGrid)
    ...
    This behavior is not coincidental: Each fish produced over the time interval will be added to the value of the objective function, either as part of a harvest, or at the end of the time interval when the pond is emptied.
    In other words, fish that have been produced can never disappear. Therefore, to maximize the total amount of fish produced, it is sufficient to maximize the population growth over the entire time interval.
    
    The population growth is maximal when the population is at half of the total capacity of the pond, i.e.~$P(t) = \frac{C}{2}$. The population growth decreases quadratically with the distance of the population from the optimal point.
    Thus, our goal is to keep the population as close to the optimal point $\frac{C}{2}$ as possible.
    
    We know that our choice of parameters $T$ and $\alpha$ will constrain the population to the interval \\$J = [T, T - \alpha T]$ after the first harvest.
    Since the population growth decreases quadratically with the distance of the population from the optimal point, we should choose $T$ and $\alpha$ such that the interval $J$ has minimal length and is centered at $\frac{C}{2}$.
    
    Thus, choosing $\alpha^* = 0.2$ minimal (under the constraint $\alpha \geq 0.2$) will minimize the length of the interval.
    The optimal harvest threshold $T^*$ can then be obtained by centering the interval at the optimal point $\frac{C}{2}$, i.e.~$m^* = T^*(1 - \frac{\alpha^*}{2}) = \frac{C}{2}$ which yields $T^* = \frac{C}{2-\alpha^*} \approx 0.56$.
    This matches the numerical results obtained by the optimizer.\\ \\
    \textbf{File:} \texttt{runTask4.m}
    \begin{lstlisting}
%% Subtask f)
%% Optimize

...
if ~isempty(fPlusFunc) || ~isempty(djump)
...
if ~isempty(djump)
    derivativeSolutionWrtSwitchingTime = derivativeSolutionWrtSwitchingTime - djump.dt(tMinus, yMinus, p, 1);

    % Evaluate jump_y in direction of sens and fMinus in one call for efficiency.
    % Last column belongs to fMinus.
    jumpDyPartial = djump.dy(tMinus, yMinus, p, [sens, fMinus]);
    ...
    if hasParameter
        ...
        update(:, nDirY+1:end) = update(:, nDirY+1:end) + djump.dp(tMinus, yMinus, p, dirP);
    end
end
...
if ~isempty(dsigma)
    derivativeSwitchingTimeWrtParams = derivativeSwitchingTimeWrtParams + dsigma.dy(tMinus, yMinus, p, sens);
    if hasParameter
        ...
        derivativeSwitchingTimeWrtParams(nDirY+1:end) = derivativeSwitchingTimeWrtParams(nDirY+1:end) ...
            + dsigma.dp(tMinus, yMinus, p, dirP);
    end
end
...
sigmaDtPartial = dsigma.dt(tMinus, yMinus, p, 1);
sigmaDyPartialFminus = dsigma.dy(tMinus, yMinus, p, fMinus);
